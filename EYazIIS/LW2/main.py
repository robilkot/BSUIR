...
# ------------------------------
# Определение классов для анализа
# ------------------------------
...
class CorpusApp(tk.Tk):
    ...
    def create_widgets(self):
        ...
        ttk.Label(criteria_frame, text="POS:").grid(row=0, column=0, padx=2, pady=2)
    ...
    def update_frequencies(self):
        ...
        for characteristic, need_plot in characteristics:
            ...
            if need_plot:
                ...
                # Создание фигуры и отрисовка столбчатого графика
                fig, ax = plt.subplots(figsize=(5, height_coef))
                ax.barh(freq[characteristic].keys(), freq[characteristic].values(), color='orange')
        ...
        if results:
            for entry_id, token in results:
                line = (f"Запись: {entry_id}, Токен: {token.text}, Лемма: {token.lemma}, "
                        f"POS: {token.pos}, Характеристики: {token.feats}\n")
