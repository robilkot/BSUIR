...
class Workspace(ttk.Frame):
    def __init__(self, parent):
        ...
        self.__open_text_button: ttk.Button = None
    ...
    def create_workspace_widgets(self):
        ...
        self.__open_text_button = ttk.Button(self, text="Открыть файл", command=fs.FileSystem.open_file)
        self.__open_text_button.pack(pady=50)

        state_footer = ttk.Frame(self, relief="groove", borderwidth=1)
        state_footer.pack(side=tk.BOTTOM, fill=tk.X, anchor=tk.W)

        words_num_label = ttk.Label(state_footer, text="Количество слов:")
        words_num_label.pack(side=tk.LEFT, anchor=tk.W)
        self.words_num_label = ttk.Label(state_footer, text="0")
