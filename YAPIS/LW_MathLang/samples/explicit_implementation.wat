(module
  (import "console" "write_int" (func $console_write_int (param i32)))
  (import "console" "write_float" (func $console_write_float (param f32)))
  (import "console" "write_bool" (func $console_write_bool (param i32)))
  (import "console" "write_string" (func $console_write_string (param i32) (param i32)))
  (import "console" "read_int" (func $console_read_int (result i32)))
  (import "console" "read_float" (func $console_read_float (result f32)))
  (import "console" "read_bool" (func $console_read_bool (result i32)))
  (import "Math" "exp" (func $Math_exp (param f32) (result f32)))
  (import "Math" "abs" (func $Math_abs (param f32) (result f32)))
  (import "Math" "tan" (func $Math_tan (param f32) (result f32)))
  (import "Math" "atan" (func $Math_atan (param f32) (result f32)))
  (import "Math" "ceil" (func $Math_ceil (param f32) (result f32)))
  (import "Math" "sin" (func $Math_sin (param f32) (result f32)))
  (import "Math" "asin" (func $Math_asin (param f32) (result f32)))
  (import "Math" "sqrt" (func $Math_sqrt (param f32) (result f32)))
  (import "Math" "floor" (func $Math_floor (param f32) (result f32)))
  (import "Math" "cos" (func $Math_cos (param f32) (result f32)))
  (import "Math" "acos" (func $Math_acos (param f32) (result f32)))
  (import "Math" "log" (func $Math_log (param f32) (result f32)))
  (memory $memory 1)
  (data (i32.const 0) "")
  (func $expl_impl_incorrect@float@float
    (param $float i32)
    return
  )
  (func $expl_impl@int@int
    (param $val i32)
    (local $i i32)
    i32.const 0
    local.set $i
    local.get $i
    i32.const 0
    i32.store
    block $loop_end_2
    loop $loop_start_1
      local.get $i
      i32.load
      i32.const 2
      i32.lt_s
      i32.eqz
      br_if $loop_end_2
        local.get $i
        i32.load
        call $console_write_int
      local.get $i
      local.get $i
      i32.load
      i32.const 1
      i32.add
      i32.store
      br $loop_start_1
    end
    end
    return
  )
  (func $expl_impl_incorrect@int@int
    (param $float i32)
    return
  )
  (func $expl_impl@bool@bool
    (param $val i32)
    (local $i i32)
    i32.const 4
    local.set $i
    local.get $i
    i32.const 0
    i32.store
    block $loop_end_4
    loop $loop_start_3
      local.get $i
      i32.load
      i32.const 3
      i32.lt_s
      i32.eqz
      br_if $loop_end_4
        local.get $i
        i32.load
        call $console_write_int
      local.get $i
      local.get $i
      i32.load
      i32.const 1
      i32.add
      i32.store
      br $loop_start_3
    end
    end
    return
  )
  (func $main
    (local $$temp_8 i32)
    (local $$temp_7 i32)
    (local $$temp_6 i32)
    (local $$temp_5 i32)
    i32.const 8
    local.set $$temp_5
    local.get $$temp_5
    i32.const 1
    i32.store
    local.get $$temp_5
    call $expl_impl@bool@bool
    i32.const 12
    local.set $$temp_6
    local.get $$temp_6
    i32.const 2
    i32.store
    local.get $$temp_6
    call $expl_impl@int@int
    i32.const 16
    local.set $$temp_7
    local.get $$temp_7
    f32.const 2.0
    f32.store
    local.get $$temp_7
    call $expl_impl_incorrect@float@float
    i32.const 20
    local.set $$temp_8
    local.get $$temp_8
    i32.const 2
    i32.store
    local.get $$temp_8
    call $expl_impl_incorrect@int@int
    f32.const 2.0
    call $console_write_float
    f32.const 2.0
    call $Math_sin
    drop
  )
  (export "main" (func $main))
  (start $main)
)