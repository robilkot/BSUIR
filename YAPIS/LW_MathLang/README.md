...
Исходный код -> синтаксический анализ -> семантический анализ -> генерация AST -> генерация WAT и JS кода

Реализованы встроенные типы float (32 bit), int (32 bit), bool.
Неявно используются типы void и string.

Например, можно использовать строковые константы для вывода информации. Использовать строки как операнды не допускается.

Тип void применяется как тип возвращаемого значения пользовательских подпрограмм. Это необходимо, так как стандартные подпрограммы могут возвращать значения других типов, но по заданию варианта пользовательские подпрограммы допускают только передачу параметров по ссылке, без возвращаемых значений.
...
Отвечает за форматирование всех сообщений о семантических ошибках

### SemanticError

Представляет семантическую ошибку с описанием и местом в коде

### Symbol

Представляет символы: переменные, подпрограммы

### SubprogramSymbol

Представляет символ подпрограммы

### SymbolTable

Представляет таблицу символов области видимости

### TypeChecker

Выполняет проверки типов, преобразований, находит типы результатов операций

### Type

Представляет типы (как стандартные, так и шаблонные)

### SemanticAnalyzer(MathLangVisitor)

Класс, инкапсулирующий логику обхода дерева ANTLR и построение AST после семантических проверок

### WatGenerator

Обходит AST и генерирует код WAT

### ProgramNode и другие *Node
