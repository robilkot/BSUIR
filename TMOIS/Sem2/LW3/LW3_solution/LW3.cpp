...
void showGraphic(string operation, const Graphic& result) //Функция вывода множества в консоль
{
...
void createGraphic(Graphic& vec, char x) //Функция создания множества с вводом элементов пользователем
{
...
void unite(const Graphic& A, const Graphic& B, Graphic& D) //Функция объединения множеств
{
	D.clear();
	...
	for (const auto& b : B) { // Проходим по всем элементам множества B
		...
		for (const auto& d : D) { // Для каждого выбранного элемента мн-ва B проходим по всем элементам множества D
			if (b == d) {
				ignore = true;	// и сравниваем выбранные элементы.
				break;
			}
		}
...
void intersect(const Graphic& A, const Graphic& B, Graphic& D) // Операция пересечения множеств
{
	D.clear();
...
void difference(const Graphic& A, const Graphic& B, Graphic& D) { // Функция нахождения разности множеств ---TODO Фикс выхода за пределы (где?)
	D.clear();
	bool notinB;
...
void symdifference(const Graphic& A, const Graphic& B, Graphic& D) { // Функция нахождения симметрической разности множеств
	D.clear();
	Graphic diffAB, diffBA;
...
void complement(const Graphic& U, const Graphic& A, Graphic& D) { // Функция нахождения дополнения множества
	D.clear();
	difference(U, A, D); // Находим разность универсума и данного множества
}

void inversion(const Graphic& A, Graphic& D) { // Функция нахождения инверсии графика
	D.clear();
	D = A;
	for (auto& x : D)
...
void composition(const Graphic& A, const Graphic& B, Graphic& D) { // Функция нахождения симметрической разности множеств
	D.clear();
...
int main()
{
	...
	Graphic D, U; // Создаем универсум и далее заполняем
	unite(A, B, U);
	showGraphic("view", U);
